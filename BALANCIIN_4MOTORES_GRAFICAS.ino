...
float PRateRoll = 1.5;      //3.8, 1.7
float IRateRoll = 0.00012;  //1.2
float DRateRoll = 0.088;    //0.0088
...
uint32_t LoopTimer;
float t = 0.008;  //time cycle: 0.012
...
void setup(void) {
  ...
  RateCalibrationRoll = -0.55;
  RateCalibrationPitch = -0.63;
  RateCalibrationYaw = 0.03;
...
void loop(void) {
  ...
  // // Inlined Kalman Filter computation in the loop   ACTIVAMOS AQUIII
  KalmanAngleRoll += t * RateRoll;
  KalmanUncertaintyAngleRoll += t * t * 20;                                        // Variance of IMU (4 deg/s) squared// MOVI DE 16
  KalmanGainRoll = KalmanUncertaintyAngleRoll / (KalmanUncertaintyAngleRoll + 5);  // Error variance (3 deg) squared
  ...
  /////HASTA AQUI ACTIVAMOS EL FILTRO KALMAN
