//CODIGO WIFI PARA CAMBIAR LAS CONSTANTES DE PID
#include <WiFi.h>
#include <AsyncTCP.h>
...
Wire.beginTransmission(0x68);
  ...
  AngleRoll=atan(AccY/sqrt(AccX*AccX+AccZ*AccZ))*30; //*1/(3.142/180);
  AnglePitch=atan(AccX/sqrt(AccY*AccY+AccZ*AccZ))*30;             //REBE: SIMBOLOMOD(-)
}
...
Wire.beginTransmission(0x68);
  ...
  int16_t GyroZ=Wire.read()<<8 | Wire.read();
  RateRoll=(float)GyroX/65.5;
  RatePitch=-(float)GyroY/65.5; //SIGNO MODIFICADO POR REBE(-)
  RateYaw=(float)GyroZ/65.5;
  AccX=(float)AccXLSB/4096;
  AccY=(float)AccYLSB/4096;
  AccZ=(float)AccZLSB/4096;
...
DesiredAnglePitch=-0.1*(ReceiverValue[1]-1500); //SIGNO MODIFICADO POR REBE(+)
